...
print_stackframe(void) {
      ...
      */
     uint32_t firstEbp = read_ebp();
     uint32_t firstEip = read_eip();
     uint32_t *arguments;
     int i,j;
     for(i=0; i<STACKFRAME_DEPTH; i++){
        cprintf("ebp:0x%08x, eip:0x%08x args:",firstEbp,firstEip);
        arguments = (uint32_t*)firstEbp+2;
        for(j=0; j<4; j++){
            cprintf("0x%08x",arguments[j]);
        }
        cprintf("\n");
        print_debuginfo(firstEip-1);
        firstEip = ((uint32_t*)firstEbp)[1];
        firstEbp = ((uint32_t*)firstEbp)[0];
     }
}
